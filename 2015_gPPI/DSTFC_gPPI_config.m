% DSTFC_gPPI('DSTFC_gPPI_config.m')
% rm -rf .../*/...
...
Script_Dir = '/brain/iCAN/home/tianyanqiu/SPM12_scripts/gPPI/DSTFC';

% Please specify the data server path
paralist.data_server = '/brain/iCAN/home/tianyanqiu/DST/Data/data_FC_0s_new_type_E_L';

% added by genghaiyang specify the stats server path
paralist.server_path_stats = '/brain/iCAN/home/tianyanqiu/DST/Results_individualStats/FC_individualstats_0s_new_type_E_L';%1st level
...
subjlist = fullfile(Script_Dir, 'DST_SubList_newcheck.txt');

% Please specify the stats folder name (eg., stats_spm8)
paralist.stats_folder = '/FC/stats_spm8_swcar';

% get ROI file list
ROI_form = 'nii';
paralist.roi_nii_folder = '/brain/iCAN/home/tianyanqiu/SPM12_scripts/gPPI/DSTFC/ROI/nii';
...
paralist.tasks_to_include = {'1', 'CSppE', 'CSppL', 'CSpuE','CSpuL', 'CSmE', 'CSmL', 'UCS'};

% Please specify the confound names
paralist.confound_names = {'R1', 'R2', 'R3', 'R4', 'R5', 'R6'};

% make T contrast
    Pcon.Contrasts(1).left      = {'CSppE'};
    Pcon.Contrasts(1).right     = {'none'};
    Pcon.Contrasts(1).STAT      = 'T';
    Pcon.Contrasts(1).Weighted  = 0;
  % Pcon.Contrasts(1).MinEvents = 5;
    Pcon.Contrasts(1).name      = 'CSppE';
    
    Pcon.Contrasts(2).left      = {'CSppL'};
 	  Pcon.Contrasts(2).right     = {'none'};
    Pcon.Contrasts(2).STAT      = 'T';
    Pcon.Contrasts(2).Weighted  = 0;
  % Pcon.Contrasts(2).MinEvents = 5;
    Pcon.Contrasts(2).name      = 'CSppL';
    
    Pcon.Contrasts(3).left      = {'CSpuE'};
   	Pcon.Contrasts(3).right     = {'none'};
    Pcon.Contrasts(3).STAT      = 'T';
    Pcon.Contrasts(3).Weighted  = 0;
  % Pcon.Contrasts(3).MinEvents = 5;
    Pcon.Contrasts(3).name      = 'CSpuE';
    
    Pcon.Contrasts(4).left      = {'CSpuL'};
 	  Pcon.Contrasts(4).right     = {'none'};
    Pcon.Contrasts(4).STAT      = 'T';
    Pcon.Contrasts(4).Weighted  = 0;
  % Pcon.Contrasts(4).MinEvents = 5;
    Pcon.Contrasts(4).name      = 'CSpuL';

    Pcon.Contrasts(5).left      = {'CSmE'};
   	Pcon.Contrasts(5).right     = {'none'};
    Pcon.Contrasts(5).STAT      = 'T';
    Pcon.Contrasts(5).Weighted  = 0;
  % Pcon.Contrasts(5).MinEvents = 5;
    Pcon.Contrasts(5).name      = 'CSmE';
    
    Pcon.Contrasts(6).left      = {'CSmL'};
   	Pcon.Contrasts(6).right     = {'none'};
    Pcon.Contrasts(6).STAT      = 'T';
    Pcon.Contrasts(6).Weighted  = 0;
  % Pcon.Contrasts(6).MinEvents = 5;
    Pcon.Contrasts(6).name      = 'CSmL';
    
    Pcon.Contrasts(7).left      = {'UCS'};
    Pcon.Contrasts(7).right     = {'none'};
    Pcon.Contrasts(7).STAT      = 'T';
    Pcon.Contrasts(7).Weighted  = 0;
  % Pcon.Contrasts(7).MinEvents = 5;
    Pcon.Contrasts(7).name      = 'UCS';
    
    Pcon.Contrasts(8).left      = {'CSppE'};
 	  Pcon.Contrasts(8).right     = {'CSmE'};
    Pcon.Contrasts(8).STAT      = 'T';
    Pcon.Contrasts(8).Weighted  = 0;
  % Pcon.Contrasts(8).MinEvents = 5;
    Pcon.Contrasts(8).name      = 'CSppE_CSmE';
    
    Pcon.Contrasts(9).left      = {'CSppL'};
   	Pcon.Contrasts(9).right     = {'CSmL'};
    Pcon.Contrasts(9).STAT      = 'T';
    Pcon.Contrasts(9).Weighted  = 0;
  % Pcon.Contrasts(9).MinEvents = 5;
    Pcon.Contrasts(9).name      = 'CSppL_CSmL';
    
    Pcon.Contrasts(10).left      = {'CSpuE'};
 	  Pcon.Contrasts(10).right     = {'CSmE'};
    Pcon.Contrasts(10).STAT      = 'T';
    Pcon.Contrasts(10).Weighted  = 0;
  % Pcon.Contrasts(10).MinEvents = 5;
    Pcon.Contrasts(10).name      = 'CSpuE_CSmE';

    Pcon.Contrasts(11).left      = {'CSpuL'};
   	Pcon.Contrasts(11).right     = {'CSmL'};
    Pcon.Contrasts(11).STAT      = 'T';
    Pcon.Contrasts(11).Weighted  = 0;
  % Pcon.Contrasts(11).MinEvents = 5;
    Pcon.Contrasts(11).name      = 'CSpuL_CSmL';
