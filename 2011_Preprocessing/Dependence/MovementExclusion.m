function MovementExclusion(Config_File)
...
%--------------------------------------------------------------------------
Subjects   = SubjList;   % ReadList(SubjList);
NumSubjs   = length(Subjects);
Conditions = CondList;   % ReadList(CondList);
...
for iSubj = 1:NumSubjs
  for iCond = 1:NumConds
    ...
    UnnormDir = fullfile(ServerPath, YearId, Subjects{iSubj}, 'fmri', Conditions{iCond}, 'unnormalized');
    % unix(sprintf('gunzip -fq %s', fullfile(UnnormDir, '*.gz')));
    ImgFilegz = fullfile(UnnormDir, 'I.nii.gz');
    if exist(ImgFilegz, 'file') == 7
    ...
    fprintf('---> Subject: %s | Task: %s | VoxelSize: %f\n', Subjects{iSubj}, Conditions{iCond}, VoxSize);
    % unix(sprintf('gzip -fq %s', fullfile(UnnormDir, 'I*')));
    
    MvmntDir{RunCnt} = fullfile(ServerPath, YearId, Subjects{iSubj}, 'fmri', Conditions{iCond}, PrepFolder);
    ...
    if exist(ZippedMvmntFile, 'file') || exist(ZippedGSFile, 'file')
      ...
      if ~exist(MvmntFile, 'file') || ~exist(GSFile, 'file')
        ...
        CondStatsFile = fullfile(ServerPath, YearId, Subjects{iSubj}, 'fmri', Conditions{iCond}, PrepFolder, 'MovementStats.txt');
        fid = fopen(CondStatsFile, 'w+');
        fprintf(fid, '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n', ...
          'TASK', 'Scan_ID', ...
          'Range x', 'Range y', 'Range z', ...
          'Range pitch', 'Range roll', 'Range yaw', ...
          'Max Displacement', 'Max Scan-to-Scan Displacement', 'Mean Scan-to-Scan Displacement', ...
          'Num Scans > 0.5 Voxel Displacement', 'Num Scans > 5% Global Signal', '% of Volumes Repaired');
        fprintf(fid, '%s\t%s\t', Conditions{iCond}, Subjects{iSubj});
        fprintf(fid, '%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n', ...
          MvmntStats(RunCnt, 1), MvmntStats(RunCnt,  2), MvmntStats(RunCnt,  3), MvmntStats(RunCnt,  4), ...
          MvmntStats(RunCnt, 5), MvmntStats(RunCnt,  6), MvmntStats(RunCnt,  7), MvmntStats(RunCnt,  8), ...
          MvmntStats(RunCnt, 9), MvmntStats(RunCnt, 10), MvmntStats(RunCnt, 11), MvmntStats(RunCnt, 12));
        fclose(fid);
      end
    end
    RunCnt = RunCnt + 1;
...
if ~isempty(FullRunIndex)
  ...
  for i = 1:length(FullRunIndex)
    ...
    fprintf(fid, '%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n', ...
      MvmntStats(RunIndex(FullRunIndex(i), 3),  1), MvmntStats(RunIndex(FullRunIndex(i), 3),  2), MvmntStats(RunIndex(FullRunIndex(i), 3),  3), ...
      MvmntStats(RunIndex(FullRunIndex(i), 3),  4), MvmntStats(RunIndex(FullRunIndex(i), 3),  5), MvmntStats(RunIndex(FullRunIndex(i), 3),  6), ...
      MvmntStats(RunIndex(FullRunIndex(i), 3),  7), MvmntStats(RunIndex(FullRunIndex(i), 3),  8), MvmntStats(RunIndex(FullRunIndex(i), 3),  9), ...
      MvmntStats(RunIndex(FullRunIndex(i), 3), 10), MvmntStats(RunIndex(FullRunIndex(i), 3), 11), MvmntStats(RunIndex(FullRunIndex(i), 3), 12));
  end
  fclose(fid);
  
  if length(FullRunIndex) < NumRuns
   % fid = fopen('MovementMissingInfo.txt', 'w+');
   fid = fopen(['Log_MovementMissingInfo_',CondList{1,1},'.txt'], 'w+');
