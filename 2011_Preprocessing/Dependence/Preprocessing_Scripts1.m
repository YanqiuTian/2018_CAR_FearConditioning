function Preprocessing_Scripts1(funcdir,funcfilter,t1dir,t1filter,sliceorder,tr,DataType)
% inputs:
...
% 2016-9-24
[funcimages, ~] = SelectFiles(funcdir, funcfilter, DataType);
...
if strfind(spmver,'SPM12')
    ...
    clear matlabbatch;
    load(fullfile(a,'Preprocessing_Batch1.mat'));
    % a-Slice Timing
    ...
    % matlabbatch{2}.spm.  % leave default? 2017-09-23 Bingsen
    ...
    % calculate global
    [rfiles,~] = SelectFiles(funcdir, 'carI', DataType);
    VY = spm_vol(rfiles);
    NumScan = length(VY);
