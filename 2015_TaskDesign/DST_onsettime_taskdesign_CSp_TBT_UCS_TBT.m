...
%     durations_UCS=[ones(size(onset_UCS,1), 1)*0];
    ...
    onset_CSp=matrix(matrix(:,5)==11, 2);
    durations_CSp=matrix(matrix(:,5)==11, 3);
...
cd(OutPath);
    ...
    fp=fopen('taskdesign_TBT_UCSTBT_CSp.m','w');
    fprintf(fp, '%s\n', 'sess_name =''FC'';');
 ...
 n=21; % CSp
    ...
    for m=1:n
        fprintf(fp, '%s\n',strcat('names{',num2str(m),'}   = [''CSp_',num2str(m),'''];'));
        fprintf(fp, strcat('onsets{',num2str(m),'} = [',num2str(onset_CSp(m)),'];\n'));
        fprintf(fp, strcat('durations{',num2str(m),'} = [',num2str(durations_CSp(m)),'];\n'));
    end
...
%         'durations{37}    = [', repmat('%f ', 1, length(durations_UCS)), '];\n'], durations_UCS{:});
    ...
    fprintf(fp, '%s\n','rest_exists  = 1;');
    fprintf(fp, '%s\n','save taskdesign_TBT_UCSTBT_CSp.mat sess_name names onsets durations rest_exists');
