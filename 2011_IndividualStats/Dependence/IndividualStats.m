...
for subcnt = 1:numsub
    ...
    for sesscnt = 1:numsess
        ...
        if(include_mvmnt == 1)
            load task_design % modified with specific taskdesign name
    ...
    if include_artrepair == 1
        addpath(genpath('/brain/iCAN/home/tianyanqiu/SPM/spm12/toolbox/ArtRepair'));  % Bingsen
...
%load(fullfile(template_path,'batch_stats_noderiv.mat'));  % Shaozheng added
 load(fullfile (template_path,'IndividualStats_Batch_scans.mat')); % for scan unit % Bingsen
% load(fullfile (template_path,'IndividualStats_Batch.mat'));%for scecond unit
...
% matlabbatch{1}.spm.stats.fmri_spec.global = 'Scaling'   % original value is 'None' %global normalization
